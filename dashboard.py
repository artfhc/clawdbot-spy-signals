...
st.set_page_config(page_title="SPY Trading Dashboard", page_icon="📈", layout="wide")
...
def load_intl_data(period="2y"):
    ...
    try:
        data = yf.download(
            "VEU", period=period, progress=False
    ...
    try:
        for ticker, name in sectors.items():
            ...
            if data is not None and len(data) > 0:
                ...
                ma200 = (
                    data["Close"].rolling(200).mean().iloc[-1]
                    if len(data) >= 200
                    else ma50
                )
                ...
                ret_1m = (
                    (data["Close"].iloc[-1] / data["Close"].iloc[-21] - 1) * 100
                    if len(data) >= 21
                    else 0
                )
...
def get_strategy_position(signals, row=None, intl_mom=None, rsi_in_state=None):
    ...
    positions = {
        ...
        "Signal Stacker": (
            2.0 if count >= 5 else (1.5 if count >= 4 else (1.0 if count >= 3 else 0.0))
        ),
        "Steady Eddie": (
            2.0 if count == 5 else (1.5 if count >= 4 else (1.0 if count >= 3 else 0.0))
        ),
    ...
    if row is not None:
        ...
        rsi_pos = (
            1.0 if (above_200 and rsi_oversold) else (0.0 if rsi_recovered else 0.5)
        )
...
def run_backtest(
    data, strategy_name, initial_capital=100000, intl_data=None, bond_data=None
):
    ...
    if intl_data is not None:
        ...
        intl_df = intl_df[["Mom12M", "Close"]].rename(
            columns={"Mom12M": "Intl_Mom12M", "Close": "Intl_Close"}
        )
    ...
    for idx, row in df.iterrows():
        ...
        intl_mom = (
            row["Intl_Mom12M"]
            if "Intl_Mom12M" in row and pd.notna(row["Intl_Mom12M"])
            else None
        )
    ...
    if strategy_name == "Dual Momentum" and "Intl_Return" in df.columns:
        df["Strategy_Return"] = df.apply(
            lambda r: r["Position"]
            * (r["Intl_Return"] if r["Position_Type"] == "INTL" else r["Return"]),
            axis=1,
        )
    else:
    ...
    strategy_cagr = (
        ((df["Strategy"].iloc[-1] / initial_capital) ** (1 / years) - 1) * 100
        if years > 0
        else 0
    )
    buyhold_cagr = (
        ((df["Buy_Hold"].iloc[-1] / initial_capital) ** (1 / years) - 1) * 100
        if years > 0
        else 0
    )
...
if data is None or len(data) == 0:
    ...
    st.info(
        "This can happen due to API rate limits or network issues. Try refreshing the page."
    )
    st.stop()
...
intl_mom = (
    intl_data["Mom12M"].iloc[-1]
    if intl_data is not None and len(intl_data) > 0
    else None
)
...
with col4:
    st.metric(
        ...
        delta=(
            "Bullish"
            if signal_count >= 4
            else ("Neutral" if signal_count >= 3 else "Bearish")
        ),
    )
...
with st.expander("📊 Sector Heatmap", expanded=False):
    ...
    if sector_data:
        ...
        for ticker, info in sector_data.items():
            trend = (
                "🟢"
                if info["above_50ma"] and info["above_200ma"]
                else ("🟡" if info["above_50ma"] else "🔴")
            )
            sector_list.append(
                {
                    "Sector": info["name"],
...
all_strategies = (
    list(strategy_info_original.keys())
    + list(strategy_info_new.keys())
    + list(strategy_info_hybrid.keys())
)
chart_strategy = st.selectbox(
    "Show buy/sell signals for:", all_strategies, index=len(all_strategies) - 3
)  # Default to Ensemble
...
def get_trade_signals(data, strategy_name, intl_data=None):
    ...
    for i, (idx, row) in enumerate(data.iterrows()):
        ...
        if intl_data is not None and idx in intl_data.index:
            intl_mom = (
                intl_data.loc[idx, "Mom12M"]
                if pd.notna(intl_data.loc[idx, "Mom12M"])
                else None
            )
...
buy_dates, buy_prices, sell_dates, sell_prices = get_trade_signals(
    data, chart_strategy, intl_data
)
...
fig.add_trace(
    go.Scatter(
        x=data.index, y=data["MA20"], name="MA20", line=dict(color="orange", width=1)
    ),
    row=1,
    col=1,
)
fig.add_trace(
    go.Scatter(
        x=data.index, y=data["MA50"], name="MA50", line=dict(color="blue", width=1)
    ),
    row=1,
    col=1,
)
fig.add_trace(
    go.Scatter(
        x=data.index, y=data["MA200"], name="MA200", line=dict(color="red", width=2)
    ),
    row=1,
    col=1,
)
...
# RSI
fig.add_trace(
    go.Scatter(x=data.index, y=data["RSI"], name="RSI(14)", line=dict(color="purple")),
    row=2,
    col=1,
)
fig.add_trace(
    go.Scatter(
        x=data.index, y=data["RSI5"], name="RSI(5)", line=dict(color="cyan", dash="dot")
    ),
    row=2,
    col=1,
)
...
if run_btn:
    with st.spinner("Running backtest..."):
        bt_data, metrics = run_backtest(
            data, selected_strategy, initial_capital, intl_data, bond_data
        )
...
with ps_col1:
    portfolio_size = st.number_input(
        "Portfolio Size ($)", value=100000, step=5000, min_value=1000
    )
...
for strat in [
    ...
    if pos == 0:
        ...
        etf = f"SPY + SSO (50/50)"
        shares = (
            f"{int(spy_alloc/spy_price)} SPY + {int(sso_alloc/sso_price_approx)} SSO"
        )
...
st.caption(
    "💡 Tip: SSO is 2x leveraged SPY. For 1.5x exposure, split 50/50 between SPY and SSO."
)
...
with st.expander("Track Your Portfolio", expanded=False):
    ...
    with track_col1:
        ...
        spy_avg_cost = st.number_input(
            "SPY Avg Cost ($)", value=0.0, min_value=0.0, step=1.0
        )

    with track_col2:
        sso_shares = st.number_input("SSO Shares", value=0, min_value=0, step=1)
        sso_avg_cost = st.number_input(
            "SSO Avg Cost ($)", value=0.0, min_value=0.0, step=1.0
        )

    with track_col3:
        cash_balance = st.number_input(
            "Cash Balance ($)", value=0.0, min_value=0.0, step=100.0
        )
    ...
    if spy_shares > 0 or sso_shares > 0 or cash_balance > 0:
        ...
        total_pnl_pct = (
            (total_pnl / (spy_cost_basis + sso_cost_basis) * 100)
            if (spy_cost_basis + sso_cost_basis) > 0
            else 0
        )
        ...
        st.caption(
            f"Allocation: SPY {spy_pct:.1f}% | SSO {sso_pct:.1f}% | Cash {cash_pct:.1f}%"
        )
...
with st.expander("Check Rebalancing Needs", expanded=False):
    ...
    with reb_col1:
        target_strategy = st.selectbox(
            "Target Strategy", all_strategies, key="rebal_strat"
        )
    ...
    with reb_col2:
        ...
        st.metric(
            "Target SPY", f"{target_spy}%", delta=f"{target_spy - current_spy_pct:+d}%"
        )
        st.metric(
            "Target SSO", f"{target_sso}%", delta=f"{target_sso - current_sso_pct:+d}%"
        )
        st.metric(
            "Target Cash",
            f"{target_cash}%",
            delta=f"{target_cash - current_cash_pct:+d}%",
        )

    # Calculate drift
    total_drift = (
        abs(target_spy - current_spy_pct)
        + abs(target_sso - current_sso_pct)
        + abs(target_cash - current_cash_pct)
    )
...
st.divider()
st.caption(
    f"Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Data from Yahoo Finance"
)
st.caption(
